...
class MainWindow(QMainWindow):
    ...
    def __init__(self, repository: HistoryRepository | None = None, session: GameSession | None = None) -> None:
        ...
        nav.addSpacing(35)
        self.nav_buttons = []
        self.page_titles = ["天下舆图", "起居与朝政", "史料书库", "诏令与起居簿", "待议事项", "皇帝档案"]
        ...
        main.addWidget(label("框架预览：已接入时间、指令流程与皇帝档案；人口、钱粮、忠心等国家数值尚未模拟。", "notice"))
        ...
        self.emperor_page.activity_draft_provider = lambda: [combo.currentData() for combo in self.activity_combos]
    ...
    def order_for_selected_region(self) -> None:
        ...
        self._message("已选择目标地区。请先安排处理朝政、用完本旬行动力，再进入朝政拟旨。")
    ...
    def refresh(self) -> None:
        ...
        if advice:
            ...
            self.decision_text.setText(f"{event.title}\n{event.description}\n" + ("本旬已安排朝政，无额外行动力成本。" if state.court_assigned else "本旬未安排朝政，须终止一个活动。") + "应急命令可透支诏书，下旬扣还。")
        ...
        path = project_root() / "docs" / "待确认事项.md"
        if path.exists():
    ...
    def inject_emergency(self, _checked=False, *, next_turn=False) -> None:
        ...
        if any(event.demo and event.status in {"pending", "active"}
            ...
            return
        if (not next_turn and state.phase == Phase.EXECUTING and not state.court_assigned
        ...
        self.refresh()
        self._message("已安排下一旬的演示急报。" if next_turn else
