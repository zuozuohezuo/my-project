"""Strict, JSON-safe county economy data validation. All quantities are demo units."""
...
AGES = {"child": "幼少", "adult": "成年", "elder": "老年"}
...
def validate_rules(rules: Any) -> None:
    mapping(rules, {"history_limit", "household_size", "age_labor", "needs", "wealth_effects",
    ...
    mapping(rules["age_labor"], set(AGES), "年龄劳力系数")
    for value in rules["age_labor"].values():
        number(value, "年龄劳力系数", 0, 1)
...
def validate_county(county: Any, pools: dict) -> None:
    ...
    population = mapping(county["population"], {"cohorts", "shortage_turns"}, "人口底账")
    number(population["shortage_turns"], "连续缺粮旬数", integer=True)
    cohorts = records(population["cohorts"], "人口群体")
    if not cohorts:
        raise ValueError("人口底账不能为空，零人口也须保留群体结构。")
    identities = set()
    for cohort in cohorts:
        mapping(cohort, {"id", "wealth", "age", "resident_status", "count", "health", "satisfaction",
                         "labor_factor", "death_remainder", "descent_remainder"}, "人口群体")
        text_value(cohort["id"], "人口群体编号")
        if (cohort["wealth"] not in WEALTH or cohort["age"] not in AGES
                or cohort["resident_status"] not in {"settled", "displaced"}):
            raise ValueError("人口财富、年龄或居住身份无效。")
        identity = (cohort["wealth"], cohort["age"], cohort["resident_status"])
        if identity in identities:
            raise ValueError("人口交叉群体重复。")
        ...
        for key in ("death_remainder", "descent_remainder"):
            number(cohort[key], key, 0, 1)
            if cohort[key] >= 1:
                raise ValueError("人口小数余量必须小于1。")
    _unique(cohorts, "人口群体")
    number(sum(row["count"] for row in cohorts), "县总人口", integer=True)
    for wealth, age, status in identities:
        if any((target, age, status) not in identities for target in WEALTH):
            raise ValueError("每种年龄与居住身份须有完整三档群体，允许人数为零。")
...
def validate_report(report: Any) -> None:
    mapping(report, {"turn_index", "month", "xun", "summary", "production", "consumption", "groups", "deaths",
                     "moved_down", "population_before", "population_after", "grain_shortage_ratio", "pool_changes"}, "旬报")
    ...
    for key in ("deaths", "moved_down", "population_before", "population_after"):
        number(report[key], key, integer=True)
    number(report["grain_shortage_ratio"], "粮食缺口", 0, 1)
    if report["population_before"] - report["deaths"] != report["population_after"]:
    ...
    for row in records(report["groups"], "群体报告"):
        mapping(row, {"id", "label", "population_before", "deaths", "moved_down", "mortality_risk", "health_penalty", "satisfaction_penalty", "labor_penalty"}, "群体报告")
        if row["id"] not in WEALTH:
            raise ValueError("群体报告财富层无效。")
        text_value(row["label"], "群体名称")
        for key in ("population_before", "deaths", "moved_down"):
    ...
    for key in ("deaths", "moved_down", "population_before"):
...
def validate_economy(economy: Any) -> None:
    ...
    if type(economy["schema_version"]) is not int or economy["schema_version"] != 1:
        raise ValueError("不支持的经济结构版本。")
    if type(economy["rules_version"]) is not int or economy["rules_version"] != 1 or economy["demo"] is not True:
        raise ValueError("本版仅支持规则版本1的一县演示。")
    ...
    validate_county(economy["county"], economy["pools"])
