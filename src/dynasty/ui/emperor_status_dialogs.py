...
class EmperorHealthEditor(_StatusEditor):
    ...
    def __init__(
        ...
        activity_draft: list[str | Activity | None] | None = None,
    ) -> None:
    ...
    def apply_values(self) -> None:
        ...
        if self.activity_draft is not None:
            count = sum(getattr(value, "reserved_ap", getattr(value, "cost", 1))
                        for value in self.activity_draft if value is not None)
            capacity = self.session.config.action_points(new_health)
            if count > capacity:
                self.fail(f"新健康值只提供 {capacity} 点行动力，当前活动草稿已安排 {count} 点。请先减少活动安排。")
                return
        conditions = []
